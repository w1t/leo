    ...
    fn resolve(table: &mut SymbolTable, unresolved: Self::UnresolvedNode) -> Result<Self, Self::Error> {
        ...
        Ok(match s_or_e {
            UnresolvedSpreadOrExpression::Spread(spread) => {
                let resolved_spread = Expression::resolve(table, (expected_type, spread)).unwrap();
                // TODO: add check for array type
                SpreadOrExpression::Spread(resolved_spread)
            }
            UnresolvedSpreadOrExpression::Expression(expression) => {
                let resolved_expression = Expression::resolve(table, (expected_type, expression)).unwrap();
                // TODO: add check for array type
                SpreadOrExpression::Expression(resolved_expression)
            }
        })
    }
}
