    ...
    pub fn assign_tuple<CS: ConstraintSystem<F>>(
        ...
        match self.get_mutable_assignee(name, span.clone())? {
            ConstrainedValue::Tuple(old) => {
                ...
                let selected_value =
                    ConstrainedValue::conditionally_select(cs.ns(|| name_unique), indicator, &new_value, &old[index])
                        .map_err(|_| StatementError::select_fail(new_value.to_string(), old[index].to_string(), span))?;
